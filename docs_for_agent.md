...
│   ├── subscription.js           # PLUS++ 共享逻辑（Luhn mod N 校验、卡密生成、通知推送、isUserPlus 检测、置顶月度计数）
│   └── hotness.js                # 帖子热度计算
...
### 3.13 每日发帖限额 / 匿名发帖 / 悄悄话配额系统

**每日发帖限额**（`routes/posts.js`）：
- PLUS 用户：`Infinity`（不限制每日发帖数）
- 非 PLUS 用户：每天免费 5 次发帖（自然日，0:00 重置），统计 `posts` 表 `userId + time`。
- count >= 5 时：若请求无 `payWithCredit` 参数 → 返回 `{ok:false, code:'DAILY_POST_LIMIT', cost:39}`；有 `payWithCredit=true` 且 credit >= 39 则扣除并放行；credit 不足返回 `INSUFFICIENT_CREDIT`。
- 前端弹窗确认框：2个按钮「返回编辑」「消耗 39 credit 发布」。

**帖子置顶**（`routes/posts.js` + `lib/subscription.js`）：
- 发帖时传入 `pinned: true` 可置顶帖子，置顶有效期 7 天。
- 非 PLUS 用户：扣除 100 credit（需 credit >= 100）。
- PLUS 用户：每月前 40 次免费（按自然月重置，追踪 `users.pinCount` + `users.pinMonth`），超过 40 次按非 PLUS 处理（扣 100 credit）。
- 置顶帖子在 `GET /api/posts` 中排在最前面（按 `pinnedAt` 降序），7 天后自动清除 `pinned` 和 `pinnedAt`。
- 前端发帖弹窗「更多选项」中新增置顶 checkbox + 费用提示（非 PLUS 显示 100 credit，PLUS 显示剩余免费次数）。
- 帖子卡片左上角显示金色置顶徽章（`.post-pinned-badge`）。
- 数据库：`posts` 表新增 `pinned`(INTEGER DEFAULT 0) 和 `pinnedAt`(TEXT) 列；`users` 表新增 `pinCount`(INTEGER DEFAULT 0) 和 `pinMonth`(TEXT) 列。

**点赞/评论自动通知**（`routes/posts.js`）：
- 用户 B 点赞用户 A 的帖子 → A 收到通知 `{title: '❤️ 收到点赞', content: 'B 赞了你的帖子「...」'}`。
- 用户 B 评论用户 A 的帖子 → A 收到通知 `{title: '💬 收到评论', content: 'B 评论了你的帖子「...」'}`。
- 自己点赞/评论自己的帖子不触发通知。
- 使用 `lib/subscription.js` 的 `pushUserNotice()` 函数。
...
| 表 | 关键字段 | 用途 |
|----|----------|------|
| `users` | id(PK), username(UNIQUE), password, nickname, avatar, uid, regIp, createdAt, status('active'/'banned'), postCount, bindAdminId, bindAdminRole, credit, checkedInDate, checkinStreak, banUntil, zhixueStatus, certData(加密), zhixueReviewedBy, zhixueCertType, zhixueUsername/Password, zhixueManual*, certRealName, certClassName, noticePublisher, **credibility_score**(INTEGER DEFAULT 90), **credibility_exchanged_total**(TEXT), **credibility_last_refresh**(TEXT), **bullyingProtection**(INTEGER DEFAULT 0), **pinCount**(INTEGER DEFAULT 0), **pinMonth**(TEXT) | 用户账号 + 认证 + 积分 + 信用分 + 霸凌保护 + 置顶月度计数 |
| `posts` | id(PK), content, author, avatar, userId, time, type('text'/板块), deleted, **pinned**(INTEGER DEFAULT 0), **pinnedAt**(TEXT), images(JSON), isAnonymous, likes, likedBy, comments(JSON), commentsCount, discussionId, rotate, zIndex, deletedAt, deletedBy, **visibility**(TEXT DEFAULT 'public'), **allowComments**(INTEGER DEFAULT 1) | 帖子 |
