...
# Re-patch remote server.js to add db.require and replace functions
patch_code = r'''
const fs = require('fs');
...
for (const [fn, body] of replacements) {
  ...
  if (idx === -1) {
    // Try without 'data' param
    const alt = fn.replace('(data)', '(d)');
    idx = content.indexOf('function ' + alt);
  }
  if (idx === -1) continue;
  ...
  const funcName = fn.includes('(') ? fn.slice(0, fn.indexOf('(')) : fn;
...
with c.open_sftp() as sftp:
    with sftp.open('/tmp/patch2.js', 'w') as f:
        f.write(patch_code)

si, so, se = c.exec_command(NODE + ' /tmp/patch2.js 2>&1', timeout=15)
...
# Verify
si, so, se = c.exec_command('grep -c "require.*db" /www/wwwroot/campus-wall/server.js; echo "---"; grep -c "hasAdmins.*db" /www/wwwroot/campus-wall/server.js', timeout=10)
out = so.read().decode('utf-8', errors='replace')
parts = out.split('---')
print('db require:', parts[0].strip(), '| hasAdmins db:', parts[1].strip() if len(parts)>1 else '?')
