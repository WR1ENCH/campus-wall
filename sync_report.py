...
# Re-patch (43 functions to db aliases)
patch = '''
const fs = require('fs');
...
let n=0;
for(const[fn,bd]of f){const s='function '+fn;let i=c.indexOf(s);if(i<0)continue;let a=c.indexOf(')',i);if(a<0)continue;let b=c.indexOf('{',a);if(b<0)continue;let d=1,e=b+1;while(d>0&&e<c.length){if(c[e]==='{')d++;else if(c[e]==='}')d--;e++}
const pf=fn.includes('(')?fn.split('(')[0]:fn;const pa=fn.includes('(')?fn.slice(fn.indexOf('(')):'()';c=c.replace(c.slice(i,e),'function '+pf+pa+' { '+bd+'; }');n++}
fs.writeFileSync('/www/wwwroot/campus-wall/server.js',c,'utf8');console.log('Patched '+n+' functions');
'''

with c.open_sftp() as sftp:
    with sftp.open('/tmp/p4.js', 'w') as f:
        f.write(patch)

si, so, se = c.exec_command(NODE + ' /tmp/p4.js 2>&1')
...
si, so, se = c.exec_command('grep "发送 T1" /www/wwwroot/campus-wall/server.js', timeout=10)
print('T1 notice:', '✅' if so.read().decode('utf-8', errors='replace') else '❌')
