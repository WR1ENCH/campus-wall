...
for f in ['server.js', 'bully.html']:
    print(f'Uploading {f}...')
    with c.open_sftp() as sftp:
        sftp.put(rf'C:\Users\wyxgg\Desktop\test\{f}', f'/www/wwwroot/campus-wall/{f}')
...
with c.open_sftp() as sftp:
    with sftp.open('/tmp/p5.js', 'w') as f:
        f.write(patch)

si, so, se = c.exec_command(NODE + ' /tmp/p5.js 2>&1')
...
si, so, se = c.exec_command('grep "霸凌.*T0" /www/wwwroot/campus-wall/server.js', timeout=10)
print('T0 notice:', '✅' if so.read().decode('utf-8', errors='replace') else '❌')
si, so, se = c.exec_command('grep "霸凌.*T1" /www/wwwroot/campus-wall/server.js', timeout=10)
