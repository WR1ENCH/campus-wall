...
const FEATURES = ['whisper', 'anonymous_post', 'qa', 'post', 'vote', 'wall_message'];
...
// 获取当前生效的处罚（自动过期翻转 + 返回）
// 同一用户有 T0 和 T1 时 T0 优先。T0 过期后自动激活等待队列中的 T1
function getActivePunishment(userId) {
  if (!userId) return null;
  const list = db.readPunishments().filter(p => p.userId === userId);
  let activeT0 = null, activeT1 = null;
  for (const p of list) {
    if (['overridden', 'queued', 'revoked', 'expired'].includes(p.status)) continue;
    if (p.expiresAt && Date.now() >= new Date(p.expiresAt).getTime()) {
      db.updatePunishment(p.punishmentId, { status: 'expired' });
      continue;
    }
    if (p.level === 'T0') { activeT0 = p; break; }
    if (p.level === 'T1') activeT1 = p;
  }

  // T0 已失效/过期，检查是否有入队的 T1 需要激活
  if (!activeT0) {
    const queued = list.filter(p => p.status === 'queued');
    for (const q of queued) {
      if (q.level === 'T1') {
        q.status = 'active';
        if (q.queuedAfter) delete q.queuedAfter;
        db.updatePunishment(q.punishmentId, q);
        return q;
      }
    }
    return activeT1;
