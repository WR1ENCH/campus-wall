...
function isUserPlus(userId) {
  ...
  if (!result && subs.length > 0) {
    const matchById = subs.filter(s => s.userId === userId);
    console.log('[isUserPlus] userId:', userId, 'type:', typeof userId);
    console.log('[isUserPlus] total subs:', subs.length, 'matchById:', matchById.length);
    if (matchById.length > 0) {
      const s = matchById[0];
      console.log('[isUserPlus] sub.status:', s.status, 'sub.endTime:', s.endTime, 'now:', now, 'endTime>now:', s.endTime > now);
      console.log('[isUserPlus] sub.userId type:', typeof s.userId, 'userId type:', typeof userId);
    }
  }
  return result;
}
