...
const forgotPwdCodeStore = new Map();     // key = email(lowercased), value = { code, expiresAt }
const forgotPwdRateLimit = new Map();     // key = email(lowercased), value = lastSendTimestamp
...
// 邀请码查询限流（防遍历枚举）。key=ip，value=查询时间戳数组。
const inviteCheckLimit = new Map();
const INVITE_CHECK_MIN_MS = 5000;         // 单 IP 两次查询最小间隔 5 秒
const INVITE_CHECK_HOUR_MAX = 60;          // 单 IP 每小时最多 60 次
const INVITE_CHECK_HOUR_MS = 60 * 60 * 1000;
...
setInterval(() => {
  ...
  // 邀请码查询限流记录清理
  for (const [key, timestamps] of inviteCheckLimit) {
    const filtered = timestamps.filter(ts => now - ts < INVITE_CHECK_HOUR_MS);
    if (filtered.length === 0) inviteCheckLimit.delete(key);
    else inviteCheckLimit.set(key, filtered);
  }
  // 投票 IP 限流记录清理
...
setInterval(() => {
  ...
  // 找回密码邮箱验证码过期清理（10分钟过期）
  for (const [email, entry] of forgotPwdCodeStore) {
    if (now - entry.expiresAt > 0) forgotPwdCodeStore.delete(email);
  }
  // 找回密码发送频率限制清理（60秒过期）
  for (const [email, ts] of forgotPwdRateLimit) {
    if (now - ts > 60000) forgotPwdRateLimit.delete(email);
  }
}, 60000);

module.exports = { captchaStore, postRateLimit, qrCodeStore, redeemRateLimit, cardCreateLimits, onlineUsers, loginFailures, LOGIN_WINDOW_MS, LOGIN_MAX_FAILS, captchaGrantLimit, CAPTCHA_GRANT_WINDOW_MS, CAPTCHA_GRANT_MAX, inviteCheckLimit, INVITE_CHECK_MIN_MS, INVITE_CHECK_HOUR_MAX, INVITE_CHECK_HOUR_MS, voteIpTimestamps, VOTE_IP_WINDOW_MS, VOTE_IP_SAME_SUBNET_MAX, VOTE_IP_TOTAL_UNIQUE_MAX, voteFingerprints, wallMessageRateLimit, wallMessageDayPass, emailVerificationStore, emailCodeRateLimit, forgotPwdCodeStore, forgotPwdRateLimit };
