...
function createCheckMaintenance(readMaintenance, writeMaintenance, verifySignedToken) {
  return function checkMaintenance(req, res, next) {
    ...
    try {
      ...
      if (enabled) {
        if (reqPath === '/intro.html') return next();
        ...
        if (noticeBypass) {
          if (reqPath === '/notice.html' || reqPath === '/vote.html') return next();
...
function inputSanitize(req, res, next) {
  if (req.body && typeof req.body === 'object') {
    const { avatar, manualImages, manualEmail, images, content, title, text, body, reason, answer, question, description, options, password, zhixuePassword, oldPwd, newPwd, adminPassword, newPassword, confirmPassword, oldPassword, plan, payment, cardCode, duration, email, host, port, user, pass, fromName, bio, background, ...rest } = req.body;
    req.body = {
      ...sanitizeString(rest),
