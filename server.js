...
function powVerifyToken(token) {
  ...
  if (Date.now() - parseInt(ts, 36) > POW_TTL) return false; // 与 POW_TTL 一致
  return true;
}
...
app.use((req, res, next) => {
  ...
  // API 请求：返回 JSON 错误，让前端识别并刷新重新过 PoW
  if (req.path.startsWith('/api/')) {
    return res.status(403).json({ ok: false, pow_required: true, msg: '请刷新页面重新验证' });
  }
...
<script>
  ...
  // ===== 纯 JS SHA256（同步，避免 Web Crypto API 的异步开销）=====
  function sha256(input) {
    ...
    function R(n,f){return(n>>>f)|(n<<(32-f))}
    function S0(x){return R(x,2)^R(x,13)^R(x,22)}
    function S1(x){return R(x,6)^R(x,11)^R(x,25)}
    function G0(x){return R(x,7)^R(x,18)^(x>>>3)}
    function G1(x){return R(x,17)^R(x,19)^(x>>>10)}

    // UTF-8 encode + pad
    var msg=[],l=input.length;
    ...
    for(var b=0;b<msg.length;b+=64){
      var W=new Array(64);
      for(i=0;i<16;i++)W[i]=(msg[b+i*4]<<24)|(msg[b+i*4+1]<<16)|(msg[b+i*4+2]<<8)|msg[b+i*4+3];
      for(i=16;i<64;i++)W[i]=((G1(W[i-2])+W[i-7])>>>0)+((G0(W[i-15])+W[i-16])>>>0);
      var a=H[0],e=H[1],c=H[2],d=H[3],t2=H[4],g=H[5],f=H[6],h=H[7];
      for(i=0;i<64;i++){
        var T1=((h+S1(t2)+((t2&g)^(~t2&f))+K[i]+W[i])>>>0);
        var T2=((S0(a)+((a&e)^(a&c)^(e&c)))>>>0);
        h=f;f=g;g=t2;t2=(d+T1)>>>0;d=c;c=e;e=a;a=(T1+T2)>>>0;
      }
      H[0]=(H[0]+a)>>>0;H[1]=(H[1]+e)>>>0;H[2]=(H[2]+c)>>>0;H[3]=(H[3]+d)>>>0;
      H[4]=(H[4]+t2)>>>0;H[5]=(H[5]+g)>>>0;H[6]=(H[6]+f)>>>0;H[7]=(H[7]+h)>>>0;
    }
    var out=[];
    for(i=0;i<8;i++)for(var j=24;j>=0;j-=8)out.push((H[i]>>>j)&255);
    return out;
  }
  ...
  var nonce = 0;
  var maxAttempts = Math.pow(2, difficulty) * 4;
  var started = Date.now();
  var found = false;

  function solveBatch() {
    var batchEnd = nonce + 4096; // 每批 4096 次，然后让出线程给 UI
    while (nonce < batchEnd && nonce < maxAttempts) {
      var test = prefix + nonce;
      if (countLeadingZeros(sha256(test)) >= difficulty) {
        found = true;
        ...
        setTimeout(function() { submitSolution(nonce); }, remaining);
        return;
      }
      nonce++;
    }
  ...
  async function submitSolution(n) {
    try {
      var resp = await fetch('/api/__pow_challenge', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ prefix: prefix, nonce: '' + n }),
